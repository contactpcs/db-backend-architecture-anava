"""Add 'patient' to appointment_audit_logs' INSERT policy — claim_slot and
cancel_own both write this log with changed_by_role='patient' whenever a
patient claims a slot or cancels their own appointment (assert_owns_profile
already gates both before the write). Same bug class 25/26/44 already fixed
elsewhere for other roles; 'patient' itself was simply never added. Found
live: a real patient claim_slot's appointment UPDATE succeeded, then the
audit-log INSERT in the same transaction raised InsufficientPrivilegeError
under FORCE ROW LEVEL SECURITY, rolling the whole claim back.

Runs SQL/v1/56_audit_log_patient_role_rls.sql.

NUMBERING NOTE: same gap 0037's docstring describes.
...
def downgrade() -> None:
    op.execute("DROP POLICY IF EXISTS rls_apal_insert ON core.appointment_audit_logs")
    op.execute(
        "CREATE POLICY rls_apal_insert ON core.appointment_audit_logs FOR INSERT TO public "
        "WITH CHECK (rls_user_role() = ANY (ARRAY['super_admin', 'clinic_admin', 'doctor', "
        "'clinical_assistant', 'receptionist', 'system']))"
    )
