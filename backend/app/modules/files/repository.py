...
class EegFileRepository:
    ...
    async def create(self, *, patient_id: UUID, clinic_id: UUID, performed_by: UUID, eeg_type: str,
    ...
    async def list_for_patient(self, patient_id: UUID) -> list[dict]:
        rows = (await self.session.execute(text("SELECT * FROM patient_eeg_files WHERE patient_id = :pid ORDER BY created_at DESC"), {"pid": str(patient_id)})).mappings().all()
    ...
    async def review(self, eeg_id: UUID, *, reviewed_by: UUID, clinical_findings, is_abnormal, status: str) -> dict | None:
        return await fetch_optional(
            ...
            {"reviewed_by": str(reviewed_by), "findings": clinical_findings, "abnormal": is_abnormal, "status": status, "id": str(eeg_id)},
        )
...
class MedicalHistoryFileRepository:
    ...
    async def create(self, *, patient_id: UUID, clinic_id: UUID, uploaded_by: UUID, document_type: str,
    ...
    async def get(self, mhf_id: UUID) -> dict | None:
        return await fetch_optional(self.session, text("SELECT * FROM patient_medical_history_files WHERE mhf_id = :id"), {"id": str(mhf_id)})
    ...
    async def soft_delete(self, mhf_id: UUID, *, deleted_by: UUID) -> dict | None:
        return await fetch_optional(
            self.session,
            text("UPDATE patient_medical_history_files SET is_deleted = TRUE, deleted_by = :by, deleted_at = NOW() WHERE mhf_id = :id RETURNING *"),
