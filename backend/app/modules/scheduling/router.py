...
async def create_weekly_schedule(
    doctor_id: UUID, body: s.WeeklyScheduleCreate, db=Depends(get_db), ctx: RequestContext = Depends(require_role(*_ALL_STAFF)),
):
...
async def list_weekly_schedules(
    doctor_id: UUID, db=Depends(get_db), _ctx: RequestContext = Depends(require_role(*_ALL_STAFF, "patient"))
):
...
async def create_schedule_override(
    doctor_id: UUID, body: s.ScheduleOverrideCreate, db=Depends(get_db), ctx: RequestContext = Depends(require_role(*_ALL_STAFF)),
):
    data = body.model_dump()
    data["clinic_id"] = str(data["clinic_id"])
...
async def get_availability(
    ...
    return await AvailabilityService(db).compute_range(
        doctor_id, from_date, to_date or from_date, include_unavailable=include_unavailable
    )
...
async def replace_my_schedule(
    body: s.MyWeeklyScheduleReplace, db=Depends(get_db), ctx: RequestContext = Depends(require_role("doctor"))
):
    from sqlalchemy import text
    ...
    return await WeeklyScheduleService(db).replace_own(
        doctor_profile_id=UUID(ctx.user_id), clinic_id=row["clinic_id"], items=items
    )


@router.post("/schedule/my/overrides", response_model=s.ScheduleOverrideRead, status_code=201)
async def create_my_override(
    body: s.MyScheduleOverrideCreate, db=Depends(get_db), ctx: RequestContext = Depends(require_role("doctor"))
):
    own = await _own_doctor_row(db, ctx)
...
async def decide_appointment_request(
    ...
    return await AppointmentRequestService(db).decide(
        request_id, body.model_dump(), reviewed_by=UUID(ctx.user_id), ctx=ctx
    )


@router.post("/appointments", response_model=s.AppointmentRead, status_code=201)
async def create_appointment(
    body: s.AppointmentCreate, db=Depends(get_db), ctx: RequestContext = Depends(require_role(*_ALL_STAFF))
):
    return await AppointmentService(db).create(
        body.model_dump(), booked_by=UUID(ctx.user_id), booked_by_role=ctx.role, ctx=ctx
    )
