...
async def list_inventory(clinic_id: UUID | None = None, product_id: UUID | None = None, db=Depends(get_db), _ctx: RequestContext = Depends(require_role(*_ALL_STAFF))):
...
async def create_stock_transfer(body: s.StockTransferCreate, db=Depends(get_db), ctx: RequestContext = Depends(require_role("super_admin", "regional_admin"))):
...
async def list_stock_transfers(to_clinic_id: UUID | None = None, status: str | None = None, db=Depends(get_db), _ctx: RequestContext = Depends(require_role(*_ALL_STAFF))):
...
async def update_stock_transfer_status(st_id: UUID, body: s.StockTransferStatusUpdate, db=Depends(get_db), ctx: RequestContext = Depends(require_role("super_admin", "regional_admin", "clinic_admin"))):
