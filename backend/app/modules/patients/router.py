...
async def register_patient(
    ...
    _ctx: RequestContext = Depends(require_role("super_admin", "clinic_admin", "receptionist")),
):
...
async def list_patients(
    registration_status: str | None = None, clinic_id: UUID | None = None,
    db=Depends(get_db), _ctx: RequestContext = Depends(require_role(*_ALL_STAFF)),
):
    return await PatientService(db).list(registration_status=registration_status, clinic_id=clinic_id)
...
async def exit_patient(patient_id: UUID, body: s.ExitInitiate, db=Depends(get_db), _ctx: RequestContext = Depends(require_role("super_admin", "clinic_admin", "doctor"))):
