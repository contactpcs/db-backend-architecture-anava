...
| **admin** | `regions`, `clinics`, `admins`, `clinic_staff_assignments`, `clinic_requests` | Region/clinic lifecycle, admin assignment, clinic request approval chain. **2026-07-04**: order is region → first clinic (auto `is_main_branch=TRUE`) → regional_admin assigned FROM that main-branch clinic (`admins.clinic_id` required, not forbidden, for `regional_admin`) → that clinic's own separate clinic_admin created after. `RegionService.assign_admin` validates the target clinic belongs to the region and is its main branch (`SQL/29_regional_admin_clinic_binding.sql`) | `clinic_created`, `clinic_status_changed`, `region_created` | `staff_onboarding_consent_signed` |
| **staff** | `doctors`, `clinical_assistants`, `ca_doctor_assignments`, `receptionists`, `staff_requests` | Staff CRUD is regional_admin/super_admin only (2026-07-03: clinic_admin can only submit a staff_request and PATCH-update existing staff, both fully audited via outbox events); CA↔Doctor assignment; hiring/removal workflow. Approving a staff_request is a pure status change — regional_admin creates the profile as a separate manual step, using an official org-domain email (see `SQL/22_staff_onboarding_lockdown.sql`) | `staff_request_approved`, `staff_onboarded`, `staff_offboarded`, `doctor_updated`/`staff_updated` | `consent_signed(staff_onboarding)` |
