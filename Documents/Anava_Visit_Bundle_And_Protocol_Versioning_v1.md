...
**Inheritance, not a fresh start.** A follow-up doesn't begin empty. If a visit has no anamnesis/PRS/protocol recorded specifically for it, the endpoint falls back to whatever was current as of that visit's date (most recent prior record for anamnesis and each PRS disease thread; the latest protocol version in the patient's open instance) and returns that instead, with `"inherited": true` on the record so the frontend can label it "carried from an earlier visit" rather than presenting it as this visit's own. This is a read-time fallback, not a copy — no new row is written just by viewing a visit. It naturally covers the initial visit's intake anamnesis too (nothing special-cased): the first visit's "current as of" resolves to version 1 by the same rule.

A doctor changes what's shown two ways, both through the existing create endpoints, no new write surface:
- **Take a new analysis / start fresh** — call anamnesis-start or PRS-start with `appointment_id` set to this visit. Once that exists, this visit's summary shows it directly (`inherited: false`) instead of falling back.
