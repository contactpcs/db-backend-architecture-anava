"""Adds a partial UNIQUE index on profiles.phone. Root-causes the "duplicate
mobile numbers in the db" report: verify-channel/confirm and signup/complete
both write the raw phone value with no ownership check, so the same number
could land on multiple profiles (confirmed live: 5 rows / 2 numbers, all one
person re-registering under different emails and re-verifying the same
mobile each time). Cleanup nulls the phone on older duplicate rows first
(keeps the newest, currently-verified one) since a UNIQUE index can't be
created while duplicates exist.

Runs SQL/v1/82_profiles_phone_unique.sql.
...
def downgrade() -> None:
    """Drops the index. The data cleanup (nulling older duplicate rows'
    phone) is not reversible — which row was "older" for a given number
    isn't recoverable once nulled."""
    op.execute('DROP INDEX IF EXISTS core."uq_profiles_phone"')
