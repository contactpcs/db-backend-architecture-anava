"""Splits reference.anamnesis_questions into registration (one-time,
asked at signup) vs main (asked every anamnesis visit after) via a new
`type` column, so GET /anamnesis-catalog can filter per flow instead of
always returning every question to both.

Runs SQL/v1/60_anamnesis_question_type.sql.
