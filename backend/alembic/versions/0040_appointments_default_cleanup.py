"""Drop the two orphan column DEFAULTs on core.appointments
(appointment_type='initial_assessment', status='scheduled') that fall
outside chk_appointments_appointment_type / chk_appointments_status (43).
Neither value is in either CHECK's allowed set, so the DEFAULT was already
dead — every real INSERT supplies both columns explicitly — this just
removes a landmine for any future INSERT that doesn't.

Runs SQL/v1/57_appointments_default_cleanup.sql.
...
def downgrade() -> None:
    op.execute("ALTER TABLE core.appointments ALTER COLUMN appointment_type SET DEFAULT 'initial_assessment'")
    op.execute("ALTER TABLE core.appointments ALTER COLUMN status SET DEFAULT 'scheduled'")
