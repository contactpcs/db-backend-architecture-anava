...
async def list_products(category: str | None = None, db=Depends(get_db), _ctx: RequestContext = Depends(require_role(*_ALL_STAFF, "patient"))):
...
async def update_store_order_status(order_id: UUID, body: s.StoreOrderStatusUpdate, db=Depends(get_db), ctx: RequestContext = Depends(require_role(*_ALL_STAFF))):
...
async def prompt_device_purchase(patient_id: UUID, clinic_id: UUID, plan_id: UUID, device_type: str, db=Depends(get_db), ctx: RequestContext = Depends(require_role(*_ALL_STAFF))):
    return await DeviceAssignmentService(db).prompt_purchase(patient_id=patient_id, clinic_id=clinic_id, plan_id=plan_id, device_type=device_type, assigned_by=UUID(ctx.user_id))
